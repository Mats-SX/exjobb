...
//		vector<int> safe_copy(g);
		...
		for (int i = 0; i < two_to_the_n2; ++i) {
			...
			*/			
			...
			int size_of_X = utils::count_1bits(x);
			...
			p.raise_to_the(size_of_X);		// p^(|X|)
			p *= utils::power_of_minus_one(n1 + n2 - size_of_X);
			g[i] *= p;
			(*pk) += g[i];
...
int main(int argc, char** argv) {
	...
	k_packing_linear_space(n1, n2, &family, &f, k, &dk, split_decision);
