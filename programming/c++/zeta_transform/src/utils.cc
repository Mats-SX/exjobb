		...
		rval_list_t* f)
{
	...
	for (int_t x1 = 0; x1 < two_to_the_n1; ++x1) {
		...
		*/
		...
		for (int_t i = 0; i < two_to_the_n2; ++i) {
			...
			/*
			cout << "fS(" << bitset<30>(x) < " = " << x << "): " << g[i] << endl;
			*/
			...
			utils::power_of_minus_one(size_of_U_minus_X) << endl;
	...
	if (true) {
		cout << "Output from FZTlin disabled" << endl;
	}
	return;
}
...
>>>>>>> Changed implementation in k-cover from vectors to int arrays in
